...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    # prepare data
    echo "Creating csv"
    sh examples/asr/st_cmds/local/prepare_data.sh || exit 1
fi

if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    # calculate cmvn
    echo "Computing cmvn"
    # cat data/train.csv > examples/asr/st_cmds/data/all.csv
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    # pretrain stage
    echo "Pretraining"
    $horovod_cmd python3 athena/${horovod_prefix}main.py \
